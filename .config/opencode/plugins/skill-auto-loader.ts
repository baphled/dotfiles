...
const SkillAutoLoaderPlugin: Plugin = async (_input) => {
  ...
  return {
    'tool.execute.before': async (input, output) => {
      ...
      // Extract category/subagent_type for skill selection
      const category = args.category as string | undefined
      let subagentType = (args.subagent_type ?? args.subagentType) as string | undefined
      
      // Model/provider routing is handled by provider-failover.ts at the chat.params hook level.
      // task() does not accept model/provider params — those properties are silently ignored.

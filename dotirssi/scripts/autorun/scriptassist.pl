...
my %IRSSI = (
  'authors' =>  'Haui, ConX',
  'contact' =>  'haui45@web.de, conx@xanthopoulos.info',
...
sub init {
  ...
  Irssi::command("statusbar window_inact disable");       # nicklist number
  $nicklist = Irssi::window_find_name("nicklist");
  Irssi::command("window $current");
}

# From trackbar.pl by Peter 'kinlo' Leurs
sub line {
    my $width  = shift;
    $string = '-';
...
sub update {
  ...
  $nicklist->print($string, MSGLEVEL_NEVER);
  $nicklist->print(line($nicklist->{'width'}), MSGLEVEL_NEVER);
